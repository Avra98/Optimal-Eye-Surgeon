...
def main(images: list, lr: float, max_steps: int, optim: str, reg: float = 0.0, sigma: float = 0.2,
    ...
    for i, file_path in enumerate(glob.glob(os.path.join(train_folder, '*.png'))):
        if i == ino:  # we start counting from 0, so the 3rd image is at index 2
            ...
            _,img_mask  = get_bernoulli_mask(img_pil,0.5)
            img_noisy_np = np.clip(img_noisy_np , 0, 1).astype(np.float32)
            img_noisy_masked  = img_mask * img_noisy_np            
    ...
    #outdir = f'data/denoising/Dataset/mask/{ino}/unet/{mask_opt}/{prior d_sigma}/{kl}'
    ...
    print(f"Output directory where results stored: {outdir}/out_sparsenet_deninp/{sigma}")
    ...
    with open(f'{outdir}/mask_{ino}.pkl', 'rb') as f:
        mask = cPickle.load(f)
    psnr,out =  train_sparse_inpaint(masked_model,net_input_list,mask, img_np, img_mask,
    ...
    with torch.no_grad():
        ...
        np.savez(f'{outdir}/out_sparsenet_deninp/{sigma}/psnr_{ino}.npz', psnr=psnr)
        

        output_paths = [
        f"{outdir}/out_sparsenet_deninp/{sigma}/out_{ino}.png",
        f"{outdir}/out_sparsenet_deninp/{sigma}/img_np_{ino}.png",
        f"{outdir}/out_sparsenet_deninp/{sigma}/img_noisy_np_{ino}.png"]
        ...
        for path, img in zip(output_paths, images_to_save):
            ...
            plt.ylabel('PSNR')
            plt.savefig(f'{outdir}/out_sparsenet_deninp/{sigma}/psnr_{ino}.png')
            plt.close()
