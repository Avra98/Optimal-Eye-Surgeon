...
def main(images: list, lr: float, max_steps: int, optim: str, reg: float = 0.0, sigma: float = 0.2,
          ...
          kl: float = 1e-5,prior_sigma: float = 0.0,var: float = 0.0,net_choice: str = "sparse"):
    ...
    def compute_fourier_power_spectrum(image):
        if image.shape[0] > 1:  # Assuming image shape is (channels, height, width)
        ...
        power_spectrum_log = np.log1p(power_spectrum)  # Using log1p for numerical stability
    ...
    if net_choice == "sparse":          
        ...
        for instance in range(1):  
            ...
            chessboard_noise = generate_chessboard_noise((3,512,512), square_size=args.square_size, noise_level=args.noise_level)
            noise = normalize_image(chessboard_noise)
            psnr, out = train_sparse(masked_model, net_input_list, mask, noise, noise, max_step=args.max_steps, show_every=args.show_every, device=args.device_id)
            print(out.shape)
            power_spectrum = compute_fourier_power_spectrum(out)
            power_spectra.append(power_spectrum)
        avg_spectrum = average_power_spectrum(power_spectra)  

        plt.imshow(out.transpose(1,2,0))  # or choose a colormap suitable for your data
        ...
        for instance in range(1):
            ...
            chessboard_noise = generate_chessboard_noise((3,512,512), square_size=args.square_size, noise_level=args.noise_level)
            noise = normalize_image(chessboard_noise)
            plt.imshow(noise.transpose(1,2,0))  # or choose a colormap suitable for your data
            plt.axis('off')
            plt.imsave('noise_chessboard.png',noise.transpose(1,2,0))
            psnr, out = train_dense(net, net_input, noise, noise, max_step=args.max_steps, show_every=args.show_every, device=args.device_id)   
            print(out.shape)
            power_spectrum = compute_fourier_power_spectrum(out)
        ...
        # plt.figure()
        # plt.title(f"PSNR for Noise var {variance}")
        # plt.plot(psnr)
        # plt.ylabel('PSNR Value')
        # #plt.savefig(os.path.join(outdir/'noise_learn'/{variance}, f"psnr_{variance}.png"))
        # plt.savefig(os.path.join(outdir, 'dense', str(variance), f"psnr_{variance}.png"))
        # plt.close()

        # # Save the PSNR results as an npz file
        # #np.savez(os.path.join(outdir/'noise_learn'/{variance}, 'psnr.npz'), psnr_lists=psnr)    
        # np.savez(os.path.join(outdir, 'dense', str(variance), 'psnr.npz'), psnr_lists=psnr)
        # ## save the psnr, output_results in the outdir folder
        # # with open(f'{outdir}/psnr_results_{ino}.pkl', 'wb') as f:   
        # noise_image_path = os.path.join(outdir, 'dense', str(variance), f"noise_{variance}.png")
        # out_image_path = os.path.join(outdir, 'dense', str(variance), f"out_{variance}.png")
        ...
        for instance in range(1):
            ...
            chessboard_noise = generate_chessboard_noise((3,512,512), square_size=args.square_size, noise_level=args.noise_level)
            noise = normalize_image(chessboard_noise)
            psnr, out = train_deep_decoder(k, noise, noise, max_step=args.max_steps, show_every=args.show_every, device=args.device_id)   
            print(out.shape)
            power_spectrum = compute_fourier_power_spectrum(out)
            print(power_spectrum.shape)
            # plt.imshow(power_spectrum)  # or choose a colormap suitable for your data
        ...
        plt.axis('off')
        plt.imsave('average_power_spectrum_deepdec.png',out.transpose(1,2,0))          
...
if __name__ == "__main__":
    ...
    parser.add_argument("--var", type=float, default=0.0, help="variance of gaussian kernel")
    parser.add_argument("--net_choice", type=str, default="sparse", help="sparse of dense")
    parser.add_argument("--square_size", type=int, default=8, help="Size of the squares in the checkerboard pattern")
    parser.add_argument("--noise_level", type=float, default=0.3, help="Noise level for the chessboard pattern")
