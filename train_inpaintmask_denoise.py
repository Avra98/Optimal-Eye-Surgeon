...
def main(images: list, lr: float, max_steps: int, optim: str, reg: float = 0.0, sigma: float = 0.2,
          ...
          ino : int =0, ino_inpaint : int =0,weight_decay: float = 0.0, mask_opt: str = "single", 
          noise_steps: int = 80000,kl: float = 1e-5,prior_sigma: float = 0.0):
    ...
    outdir = f'data/inpainting/Set14/mask/{ino_inpaint}/0.5/{mask_opt}/{prior_sigma}/{kl}'
    print(f"Output directory where net taken from: {outdir}")
    ## if it created the directory then send a print statement
    outdir_target = f'data/denoising/Set14/mask/{ino}/inpaint_mask/{ino_inpaint}/{prior_sigma}'



    m=0
    # load masked model and net_input_list from the outdir folder
    with open(f'{outdir}/masked_model_{ino_inpaint}.pkl', 'rb') as f:
        masked_model = cPickle.load(f)
    with open(f'{outdir}/net_input_list_{ino_inpaint}.pkl', 'rb') as f:
        net_input_list = cPickle.load(f)
    # load the saved mask
    with open(f'{outdir}/mask_{ino_inpaint}.pkl', 'rb') as f:
        mask = cPickle.load(f)
    ...
    with torch.no_grad():
        ...
        np.savez(f'{outdir_target}/out_sparsenet/{sigma}/psnr_{ino}.npz', psnr=psnr)
        

        output_paths = [
        f"{outdir_target }/out_sparsenet/{sigma}/out_{ino}.png",
        f"{outdir_target }/out_sparsenet/{sigma}/img_np_{ino}.png",
        f"{outdir_target }/out_sparsenet/{sigma}/img_noisy_np_{ino}.png"]
        ...
        for path, img in zip(output_paths, images_to_save):
            ...
            plt.ylabel('PSNR')
            plt.savefig(f'{outdir_target}/out_sparsenet/{sigma}/psnr_{ino}.png')
            plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ino_inpaint", type=int, default=0, help="image index  for inpainting mask taken")
    ...
    main(images=args.images, lr=args.lr, max_steps=args.max_steps, 
         ...
         show_every = args.show_every, beta = args.beta, device_id = args.device_id,ino = args.ino, ino_inpaint = args.ino_inpaint,
