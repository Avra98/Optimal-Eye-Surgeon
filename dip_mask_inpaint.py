...
def main(images: list, lr: float, max_steps: int, optim: str, reg: float = 0.0, p: float = 0.5,
          num_layers: int = 4, show_every: int=1000, device_id: int = 0,beta: float = 0.0,
          ino : int =0,weight_decay: float = 0.0, mask_opt: str = "det", noise_steps: int = 80000,
    ...
    img_noisy_np_list=[]
    noisy_psnr_list=[]
    train_folder = 'data/inpainting/Dataset'
    train_mask_folder = 'data/inpainting/Dataset/train_inpaint_{}'.format(p)
    ...
    print(img_np.shape,mask_var.shape,net_input.shape)
    net = skip(
    ...
    outdir = f'data/inpainting/Dataset/mask/{ino}/sparsity/{p}/{mask_opt}/{sparsity}/{kl}'
    print(f"Output directory: {outdir}")
    os.makedirs(f'{outdir}/out_images/', exist_ok=True)


    m=0
    p,quant_loss = learn_quantization_probabilities_dip(net,net_input, img_np, mask_var, num_steps, 
                                                        lr,ino, q=2,kl=args.kl,prior_sigma=args.prior_sigma)
    ...
    with torch.no_grad():
        ...
        f"{outdir}/out_images/img_mask_np_{ino}.png"]
            
        # print(out_np.shape, img_np.shape, img_noisy_np.shape)
        images_to_save = [out_np.transpose(1,2,0), img_np[0,:,:,:].transpose(1,2,0), img_masked.transpose(1,2,0)]
    ...
    #             plt.savefig(f"{outdir}/psnr_{}.png") 
