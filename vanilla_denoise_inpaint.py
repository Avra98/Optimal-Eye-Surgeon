...
#        img_noisy_np_list.append(img_noisy_np)  
   ...
   #[1e-1,1e-2,5e-2],[0.5,0.8] [1e-3 5e-3]
    ...
    def closure_sgd(net_input,mask,img_var,noise_var):
        mask = np_to_torch(mask).type(dtype)
        ...
        total_loss = mse(mask*out, mask*noise_var)
    ...
    outdir = f'data/denoising_inpaint/Set14/mask/{ino}/vanilla/{sigma}'
    os.makedirs(f'{outdir}', exist_ok=True)
    
    for j in range(max_steps):
        #optimizer.zero_grad()
        psnr,out = closure_sgd( net_input,mask, img_np, img_noisy_masked)
