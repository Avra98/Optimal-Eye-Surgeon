...
def main(images: list, lr: float, max_steps: int, optim: str, reg: float = 0.0, sigma: float = 0.2,
          ...
          ino : int =0, prune_epoch: int =1000, percent:float =0.1,  weight_decay: float = 0.0, mask_opt: str = "single", noise_steps: int = 80000,kl: float = 1e-5,prior_sigma: float = 0.0):
    ...
    outdir = f'data/denoising/Set14/mask/{ino}/pwt/{args.percent}_{args.prune_epoch}'
    os.makedirs(f'{outdir}/{sigma}', exist_ok=True)
    print(f"Output directory where results stored: {outdir}/{sigma}")
    #     
    psnr,out = train_and_prune_model(net, net_input, img_np, img_noisy_np, prune_type="magnitude", max_step=40000,
                                     percent=args.percent, prune_epoch=args.prune_epoch,
                                     learning_rate=1e-2, mask=None, device='cuda:0')
    ...
    with torch.no_grad():
        ...
        np.savez(f'{outdir}/{sigma}/psnr_{ino}.npz', psnr=psnr)
        

        output_paths = [
        f"{outdir}/{sigma}/out_{ino}.png",
        f"{outdir}/{sigma}/img_np_{ino}.png",
        f"{outdir}/{sigma}/img_noisy_np_{ino}.png"]
        ...
        for path, img in zip(output_paths, images_to_save):
            ...
            plt.ylabel('PSNR')
            plt.savefig(f'{outdir}/{sigma}/psnr_{ino}.png')
            plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prune_epoch", type=int, default=5000, help="interval of pruning")
    parser.add_argument("--percent", type=float, default=0.1, help="percentage of pruning")
    ...
    parser.add_argument("--kl", type=float, default=0, help="regularization strength of kl")
    ...
    main(images=args.images, lr=args.lr, max_steps=args.max_steps, 
         ...
         show_every = args.show_every, beta = args.beta, device_id = args.device_id,ino = args.ino, prune_epoch = args.prune_epoch, percent = args.percent,
